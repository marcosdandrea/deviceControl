...
class Logger {
    constructor(baseFilename, logDirectory, logHistoryInDays, logToConsole = true) {
        ...
        if (Logger.instance) {
            ...
            this.__dirname = globals.ENVIRONMENT == CONSTANTS.envionment.production ? path.join(app.getPath('userData'), this.logDirectory) : __dirname
    ...
    async #removeOldLogs() {
        ...
        const files = await fs.readdir(path.join(this.__dirname, this.logDirectory))
        ...
        for (const file of oldLogFiles) {
            await fs.unlink(path.join(this.__dirname, this.logDirectory, file))
        }
    ...
    #getCurrentLogFile() {
        ...
        return path.join(this.__dirname, this.logDirectory, `${this.baseFilename}_${year}-${month}-${day}.log`)
    }
    ...
    async #createLog(newLog) {
        ...
        try {
            await fs.mkdir(path.join(this.__dirname, this.logDirectory), { recursive: true });
    ...
    async getRoutineLogs(routineID) {
        ...
        for (const file of logFiles) {
            ...
            const thisFile = await fs.readFile(path.join(this.__dirname, this.logDirectory, file), 'utf-8')
