...
class Task extends events {
     ...
     */
    constructor({ job, condition, name, retryTimeout, retries, jobArgs, conditionArgs }) {
    ...
    run() {
        ...
        if (typeof this.job !== "function")
            throw new Error("Job must be set before running");

        if (this.condition && typeof this.condition !== "function")
            throw new Error("Condition must be set before running");
        ...
        return new Promise(async (resolve, reject) => {
            ...
            do {
                ...
                try {
                    ...
                    if (this.retry > 0) {
                        this.#eventDispatcher(taskEvents.retry, {retry: this.retry, id: this._id});
            ...
            if (this.failed) {
                //this.#eventDispatcher(taskEvents.failed, errors.join(', '));
                new Error(`Job Failed after ${this.retry} retries.`)
                reject(errors.join(', '))
            }
        })
    }
    ...
    #validateJobArgs() {
        ...
        for (let arg of requiredArgs) {
            if (!this.jobArgs.hasOwnProperty(arg.name) && arg.required) {
                return new Error(`Job requires argument "${arg.name}" with type "${arg.type}"`)
            }
    ...
    #validateConditionArgs() {
        ...
        for (let arg of requiredArgs) {
            if (!this.conditionArgs.hasOwnProperty(arg.name) && arg.required) {
                return Error(`Condition  requires argument "${arg.name}" with type "${arg.type}"`)
            }
